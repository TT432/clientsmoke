...
public final class ClientSmokeStateMachine {
    ...
    private static void handleTestExec() {
        ...
        try {
            ...
            ErrorInfo err = buildErrorInfo(reported, reported.toString());
            testResults.add(new TestResult(className, discovered.description(),
                    discovered.priority(), "failed", durationMs, err));
            LOGGER.warn("[ClientSmoke] ✗ FAIL — {} — {}", className, reported.toString());
        }
     ...
     */
    private static ErrorInfo buildErrorInfo(Throwable e, String message) {
    ...
    private static void markCurrentTestFailed(Exception e, String message) {
        Throwable reported = unwrapInvocationTarget(e);
        ErrorInfo err = buildErrorInfo(reported, message + ": " + reported.getMessage());
        if (!testResults.isEmpty()) {
            int lastIndex = testResults.size() - 1;
            TestResult previous = testResults.get(lastIndex);
            String currentClass = discoveredTests.isEmpty() || testIndex >= discoveredTests.size()
                    ? previous.className()
                    : discoveredTests.get(testIndex).className();
            if (previous.className().equals(currentClass)) {
                testResults.set(lastIndex, new TestResult(
                        previous.className(),
        ...
        if (!discoveredTests.isEmpty() && testIndex < discoveredTests.size()) {
            ClientSmokeScanner.DiscoveredTest discovered = discoveredTests.get(testIndex);
            testResults.add(new TestResult(
