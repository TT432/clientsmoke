...
/**
 * AI 调试 HTTP 服务器。端点契约：/ping /loaded /version /eval /command /enterworld /enterdworld /close。
 *
 * <p>端口经 JVM 系统属性 {@code ai_debug_port}（fallback 环境变量 {@code AI_DEBUG_PORT}）指定；
 * 两者都未配置时<b>默认不开启</b>（不绑定任何端口）。端口非法或绑定失败仅记录日志，不影响游戏进程。
 *
 * @author TT432
 */
public final class AIDebugServer {
    ...
    /** 指定调试端口的 JVM 系统属性名（{@code -Dai_debug_port=25999}）。 */
    public static final String PORT_PROPERTY = "ai_debug_port";
    /** 指定调试端口的环境变量（优先级低于系统属性），供进程启动器注入。 */
    public static final String PORT_ENV = "AI_DEBUG_PORT";

    @Nullable
    private HttpServer server;

    /**
     * 按配置启动调试服务器。读取系统属性 {@link #PORT_PROPERTY}，缺省时回退环境变量 {@link #PORT_ENV}；
     * 均未设置或端口非法时不开启。
     */
    public static void startIfConfigured() {
        ...
        if (raw == null || raw.isBlank()) {
            LOGGER.info("AI Debug Server not started (no -D{} nor env {})", PORT_PROPERTY, PORT_ENV);
            return;
        }
        final int port;
        try {
            ...
            LOGGER.error("AI Debug Server not started — invalid port '{}'", raw, e);
            return;
        }
        new AIDebugServer().start(port);
    }
    ...
    private void start(int port) {
        try {
            ...
            server.setExecutor(Executors.newFixedThreadPool(2, r -> {
                Thread t = new Thread(r, "ClientSmoke-Debug-HTTP");
                t.setDaemon(true);
                return t;
            }));
            server.start();
            ...
            LOGGER.error("Failed to start AI Debug Server on port {}", port, e);
        }
    }
